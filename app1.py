...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    temp_path = f"{pdf_id}.pdf"
    with open(temp_path, "wb") as f:
        f.write(await file.read())
    pdf_idx.build(temp_path)
    os.remove(temp_path)
    return {"ok": True, "file_url": pdf_id}
...
async def stream_answer(
    ...
    previous_history: str = Query(None)
):
    ...
    if previous_history:
        try:
            history_data = json.loads(previous_history)
            # history_data = history_data[-4:]  # Uncomment to limit history if desired
        except Exception as e:
            print("[ERROR] Could not parse previous_history:", str(e))
            history_data = []

    formatted_history = ""
    for msg in history_data:
        who = "You" if msg["role"] == "user" else "Bot"
        formatted_history += f"{who}: {msg['content']}\n"
